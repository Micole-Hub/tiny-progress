...
async function fetchWeekContextFromGoogleSheets() {
  ...
  return { currentWeek: data.currentWeek, nextWeek: data.nextWeek };
}
...
async function completeCurrentWeekInGoogleSheets() {
  const response = await fetch(GOOGLE_SHEETS_API_URL, {
    ...
    body: JSON.stringify({ secret: GOOGLE_SHEETS_API_SECRET, action: "complete-current-week" }),
  });
...
async function createItemToGoogleSheets({
  ...
  const response = await fetch(GOOGLE_SHEETS_API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ secret: GOOGLE_SHEETS_API_SECRET, action: "create", item: itemPayload }),
  });
...
async function updateItemToGoogleSheets(id, updates) {
  ...
  if (safeUpdates.subCategory !== undefined) safeUpdates.subCategory = normalizeSubCategory(safeUpdates.subCategory, safeUpdates.category || DEFAULT_CATEGORY);
  ...
  const response = await fetch(GOOGLE_SHEETS_API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ secret: GOOGLE_SHEETS_API_SECRET, action: "update", id, updates: safeUpdates }),
  });
...
async function deleteItemFromGoogleSheets(id) {
  const response = await fetch(GOOGLE_SHEETS_API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ secret: GOOGLE_SHEETS_API_SECRET, action: "delete", id }),
  });
...
function formatTasksByDifficultyForLine(tasks, difficulty) {
  ...
  if (matchedTasks.length === 0) {
    return [`📌 本週${difficulty}任務`, "", `目前沒有${difficulty}任務。`, "", "沒有案件也無妨，先喝水，本局不追殺。", "", "需要完整清單請輸入：清單"].join("\n");
  }
  ...
  return [`📌 本週${difficulty}任務`, "", ...lines, "", "以上編號沿用完整清單。", commandHint, "", "需要完整清單請輸入：清單"].join("\n");
}
...
function buildAccentBar(accentColor) {
  return { type: "box", layout: "vertical", height: "6px", backgroundColor: accentColor || FLEX_COLORS.greenFresh, cornerRadius: "999px", contents: [] };
}
...
function buildTaskFlexRow({ task, taskNumber, showDifficulty, showCategory, showSubCategory = true }) {
  ...
  const rowContents = [
    { type: "text", text: `${taskNumber}. ${checkbox} ${task.title}`, size: "sm", color: task.done ? "#8A7E6E" : FLEX_COLORS.darkGreen, wrap: true, weight: task.done ? "regular" : "bold" },
  ];

  if (tagContents.length > 0) {
    rowContents.push({ type: "box", layout: "horizontal", spacing: "xs", margin: "xs", contents: tagContents });
  }

  return { type: "box", layout: "vertical", spacing: "xs", paddingBottom: "10px", contents: rowContents };
}
...
function buildStandardFlexRow({ standard, standardNumber }) {
  ...
  return {
    ...
    spacing: "xs",
    paddingBottom: "10px",
    contents: [{ type: "text", text: `${standardNumber}. ${checkbox} ${standard.title}`, size: "sm", color: standard.done ? "#8A7E6E" : FLEX_COLORS.darkGreen, wrap: false, maxLines: 1, weight: standard.done ? "regular" : "bold" }],
  };
}
...
function buildFlexFooterHint(lines) {
  return {
    ...
    spacing: "xs",
    margin: "sm",
    contents: lines.map((line, index) => ({ type: "text", text: line, size: index === 0 ? "sm" : "xs", color: index === 0 ? FLEX_COLORS.darkGreen : "#8A7E6E", weight: index === 0 ? "bold" : "regular", wrap: true })),
  };
}

function buildBaseFlexBubble({ title, subtitle, bodyContents, footerContents, accentColor }) {
  const contents = [buildAccentBar(accentColor), buildFlexHeader(title, subtitle), { type: "separator", margin: "md", color: FLEX_COLORS.beigeLine }, ...bodyContents];
  if (footerContents) {
  ...
  return {
    type: "bubble",
    size: "mega",
    styles: { body: { backgroundColor: FLEX_COLORS.cream }, footer: { backgroundColor: FLEX_COLORS.cream } },
...
async function handleDrawOneTaskCommand() {
  ...
  if (unfinishedTasks.length === 0) {
    return ["🎲 Tiny Progress｜本局抽籤結果", "", "本週沒有未完成任務可以抽。", "如果都完成了，請給自己蓋一枚小章。", "", "可以輸入「清單」確認目前案件板。"].join("\n");
  }
  ...
  return { replyText: buildDrawOneTaskFallbackText({ selectedTask, taskNumber }), replyMessages: [buildDrawOneTaskFlexMessage({ selectedTask, taskNumber })] };
}
...
function buildDifficultyTaskListFlexMessage({ tasks, difficulty }) {
  ...
  if (matchedTasks.length === 0) {
    bodyContents = [buildFlexInfoCard([{ type: "text", text: `目前沒有${difficulty}任務。`, size: "md", color: FLEX_COLORS.darkGreen, weight: "bold", wrap: true }, { type: "text", text: "沒有案件也無妨，先喝水，本局不追殺。", size: "sm", color: FLEX_COLORS.mutedText, wrap: true }])];
  } else {
    ...
    bodyContents = [
      buildFlexInfoCard([
        ...
        ...limitedEntries.map((entry) => buildTaskFlexRow({ task: entry.task, taskNumber: entry.originalNumber, showDifficulty: false, showCategory: true })),
        ...(matchedTasks.length > limitedEntries.length ? [{ type: "text", text: `還有 ${matchedTasks.length - limitedEntries.length} 件未顯示，可輸入「清單」查看完整案件板。`, size: "xs", color: FLEX_COLORS.mutedText, wrap: true }] : []),
      ]),
    ];
  }
  ...
  const bubble = buildBaseFlexBubble({
    ...
    bodyContents,
    footerContents: buildFlexFooterHint(matchedTasks.length === 0 ? ["需要完整清單請輸入：清單", "沒有案件也算一種清爽。"] : buildDifficultyTaskFooterLines(matchedTasks, difficulty)),
  });

  return { type: "flex", altText: `Tiny Progress｜本週${difficulty}任務`, contents: bubble };
}

async function handleDifficultyTaskFlexCommand(difficulty) {
  const tasks = await getItemsByType("task");
  return { replyText: formatTasksByDifficultyForLine(tasks, difficulty), replyMessages: [buildDifficultyTaskListFlexMessage({ tasks, difficulty })] };
}
...
function buildAllListFlexMessage({ currentWeek, tasks, standards }) {
  ...
  const weekTitle = currentWeek ? `第${currentWeek.weekNumber}週｜${currentWeek.title}` : "本週案件板";
  const taskRows = tasks.slice(0, 8).map((task, index) => buildTaskFlexRow({ task, taskNumber: index + 1, showDifficulty: true, showCategory: true }));
  const standardRows = standards.slice(0, 5).map((standard, index) => buildStandardFlexRow({ standard, standardNumber: index + 1 }));

  const bodyContents = [
    buildFlexInfoCard([{ type: "text", text: weekTitle, size: "xs", color: FLEX_COLORS.mutedText, weight: "bold", wrap: false, maxLines: 1 }, buildProgressLine("任務進度", taskDoneCount, tasks.length), buildProgressLine("標準進度", standardDoneCount, standards.length)]),
  ];

  if (tasks.length === 0) {
    bodyContents.push(buildFlexInfoCard([{ type: "text", text: "本週任務尚未立案。", size: "md", color: FLEX_COLORS.darkGreen, weight: "bold", wrap: true }, { type: "text", text: "放一個小任務，就是好的開始。", size: "sm", color: FLEX_COLORS.mutedText, wrap: true }]));
  } else {
    bodyContents.push(buildFlexInfoCard([{ type: "text", text: "本週任務", size: "sm", color: FLEX_COLORS.mutedText, weight: "bold" }, ...taskRows, ...(tasks.length > taskRows.length ? [{ type: "text", text: `還有 ${tasks.length - taskRows.length} 件任務未顯示，可打開任務板查看完整內容。`, size: "xs", color: FLEX_COLORS.mutedText, wrap: true }] : [])]));
  }

  if (standards.length === 0) {
    bodyContents.push(buildFlexInfoCard([{ type: "text", text: "本週驗收標準尚未成文。", size: "sm", color: FLEX_COLORS.darkGreen, weight: "bold", wrap: true }, { type: "text", text: "寫下一個方向，慢慢前進。", size: "xs", color: FLEX_COLORS.mutedText, wrap: true }]));
  } else {
    bodyContents.push(buildFlexInfoCard([{ type: "text", text: "本週驗收標準", size: "sm", color: FLEX_COLORS.mutedText, weight: "bold" }, ...standardRows, ...(standards.length > standardRows.length ? [{ type: "text", text: `還有 ${standards.length - standardRows.length} 則標準未顯示。`, size: "xs", color: FLEX_COLORS.mutedText, wrap: true }] : [])]));
  }

  const bubble = buildBaseFlexBubble({ title: "本週案件板", subtitle: FLEX_FIXED_LINE, accentColor: FLEX_ACCENTS.all, bodyContents, footerContents: buildFlexFooterHint(["案件都在這裡，今天先辦一小件。", "需要操作說明請輸入：攻略"]) });
  return { type: "flex", altText: "Tiny Progress｜本週案件板", contents: bubble };
}

async function handleAllListFlexCommand() {
  const board = await getTaskBoardForLine();
  return { replyText: formatTaskBoardForLine(board), replyMessages: [buildAllListFlexMessage(board)] };
}
...
function normalizeLineReplyResult(replyResult) {
  if (replyResult && typeof replyResult === "object" && Array.isArray(replyResult.replyMessages)) {
    ...
    return { replyText: replyResult.replyText || getFallbackTextFromReplyMessages(replyMessages), replyMessages };
  }
  if (Array.isArray(replyResult)) return { replyText: getFallbackTextFromReplyMessages(replyResult), replyMessages: replyResult };
  const replyText = String(replyResult || "Tiny Progress 目前沒有可回覆的內容。");
  return { replyText, replyMessages: [buildLineTextMessage(replyText)] };
}
...
async function replyToLine(replyToken, replyResult) {
  ...
  const lineResponse = await fetch("https://api.line.me/v2/bot/message/reply", {
    ...
    body: JSON.stringify({ replyToken, messages: normalizedReply.replyMessages }),
  });

  if (!lineResponse.ok) console.error("LINE Reply API 回覆失敗：", lineResponse.status, await lineResponse.text());
}
...
function getUsageText() {
  return ["📮 Tiny Progress｜LINE 用量小抄", "", "你主動傳訊息，Bot 立刻回覆：", "清單、攻略、抽一件、新增、完成、修改、刪除", "", "Bot 主動傳給你：", "每日提醒、主動補提醒、主動通知", "", "目前建議：", "每天主動提醒最多 1 則。", "你主動問，本局再回。"].join("\n");
}
...
function parseCreateText({ userText, command }) {
  ...
  if (!rawText) return { title: "", category: DEFAULT_CATEGORY, subCategory: DEFAULT_SUBCATEGORY, difficulty: DEFAULT_DIFFICULTY };

  const parts = rawText.split(/\s*[\/｜|，,、]\s*/).map((part) => part.trim()).filter(Boolean);
  const title = parts[0] || "";
...
async function handleCreateCommand({ userText, command, type, label, example }) {
  ...
  if (!parsed.title) return [`這份立案公文還缺少${displayLabel}內容，本局未更動資料。`, "", `例：${command} ${example}`].join("\n");
  ...
  const createdItem = await createItemToGoogleSheets({ type, title: parsed.title, category, subCategory, difficulty, weekNumber: currentWeekNumber });
...
async function findItemByNumber({ type, numberText, label }) {
  ...
  if (!Number.isInteger(itemNumber) || itemNumber <= 0) {
    return { error: [`這份公文還缺少正確的${displayLabel}編號，本局未更動資料。`, "", "可以這樣輸入：", `完成${label}3`, `完成第三個${label}`].join("\n") };
  }

  const items = await getItemsByType(type);
  const targetItem = items[itemNumber - 1];

  if (!targetItem) return { error: [`本局目前查無第 ${itemNumber} 個${displayLabel}。`, "", "可以先輸入「清單」確認編號。"].join("\n") };
...
async function handleEditCommand({ sourceKey, numberText, newTitle, type, label }) {
  ...
  pendingActions.set(sourceKey, { action: "edit", type, label, itemNumber: result.itemNumber, itemId: result.item.id, oldTitle: result.item.title, createdAt: Date.now() });
...
function buildLineOperationCommand(actionText, targetText, numberText, newTitle) {
  ...
  if (!Number.isInteger(number) || number <= 0) return { error: "這份公文還缺少正確編號，本局未更動資料。可以這樣輸入：完成第 3 個任務" };

  const isTask = targetText.includes("任務");
  const actionMap = { 完成: "done", 已完成: "done", 取消: "cancel", 修改: "edit", 刪除: "delete" };
  return { action: actionMap[actionText], numberText: String(number), newTitle, type: isTask ? "task" : "standard", label: isTask ? "任務" : "完成標準" };
}
...
function getFormatReminderText() {
  return ["這份公文格式需補正，本局未更動資料。", "", "常用格式：", "新增任務 練習 CSS｜程式學習｜練習｜適中", "完成任務3", "取消任務3", "修改任務3", "刪除任務3", "完成標準2", "", "需要完整說明請輸入：攻略"].join("\n");
}

function getUnknownCommandText() {
  return ["Tiny Progress 目前看不懂這份公文，所以未更動資料。", "", "可以輸入：", "清單", "抽一件", "攻略", "", getLineCommandHintText()].join("\n");
}
...
async function handleLineTextCommand({ sourceKey, userText }) {
  ...
  if (userText.startsWith("新增一個任務")) return handleCreateCommand({ userText, command: "新增一個任務", type: "task", label: "任務", example: "練習 LINE Bot" });
  if (userText.startsWith("新增任務")) return handleCreateCommand({ userText, command: "新增任務", type: "task", label: "任務", example: "練習 LINE Bot" });
  if (userText.startsWith("新增一個完成標準")) return handleCreateCommand({ userText, command: "新增一個完成標準", type: "standard", label: "完成標準", example: "本週能說明一個學到的觀念" });
  if (userText.startsWith("新增一個標準")) return handleCreateCommand({ userText, command: "新增一個標準", type: "standard", label: "完成標準", example: "本週能說明一個學到的觀念" });
  if (userText.startsWith("新增標準")) return handleCreateCommand({ userText, command: "新增標準", type: "standard", label: "完成標準", example: "本週能說明一個學到的觀念" });
  ...
  if (["完成", "已完成", "取消", "修改", "刪除", "新增"].some((prefix) => userText.startsWith(prefix))) return getFormatReminderText();
...
app.post("/gas-queue", async (req, res) => {
  try {
    ...
    const normalizedReply = normalizeLineReplyResult(replyResult);
    return res.json({ ok: true, queueId, replyText: normalizedReply.replyText, replyMessages: normalizedReply.replyMessages });
  } catch (error) {
...
app.post("/weeks/complete-current", async (req, res) => {
  try {
    ...
    res.json({ message: "本週結案成功", completedWeek: result.completedWeek, currentWeek: result.currentWeek, nextWeek: result.nextWeek });
  } catch (error) {
...
app.delete("/items/:id", async (req, res) => {
  try {
    ...
    res.json({ message: "刪除成功", id: deletedItem.id, item: deletedItem });
  } catch (error) {
