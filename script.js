...
function stopApiLoading() {
  ...
  document.querySelectorAll("button").forEach((button) => {
    if (!disabledButtonsBeforeBusy.includes(button)) {
...
function renderDailyQuote() {
  ...
  dailyQuote.textContent = `本日金句：${
    DAILY_QUOTES[getDayOfYear() % DAILY_QUOTES.length]
  }`;
}

function getSelectedWeek() {
  return selectedWeekView === "next"
    ? weekContext.nextWeek
    : weekContext.currentWeek;
}

function getSelectedWeekNumber() {
  const selectedWeek = getSelectedWeek();
  return selectedWeek && selectedWeek.weekNumber
...
function renderPlanCard() {
  ...
  if (!currentWeek) {
    ...
    selectedWeekAchievement.textContent =
      "請確認後端 /week-context 是否正常，以及 weeks 工作表是否有對應日期。";
    return;
  }
  ...
  if (!selectedWeek) {
    ...
    selectedWeekAchievement.textContent = "可以先在 weeks 工作表補上下一週資料。";
    return;
  }
...
function ensureSubCategoryOptions() {
  if (!taskSubCategory) return;

  const existingValues = Array.from(taskSubCategory.options).map(
    (option) => option.value
  );

  SUBCATEGORY_OPTIONS.forEach((subCategory) => {
    if (existingValues.includes(subCategory)) return;
...
function renderAddFormState() {
  ...
  if (taskInput) {
    taskInput.placeholder = isCurrentWeekView()
  ...
  if (standardInput) {
    standardInput.placeholder = isCurrentWeekView()
...
function switchWeekView(view) {
  if (view !== "current" && view !== "next") return;
...
function normalizeDifficulty(value) {
  ...
  return DIFFICULTY_OPTIONS.includes(difficulty)
    ? difficulty
    : DEFAULT_DIFFICULTY;
}
...
function getVisibleItems() {
  ...
  return selectedWeekNumber
    ? items.filter((item) => Number(item.weekNumber) === selectedWeekNumber)
    : [];
}

function getVisibleItemsByType(type) {
  return getVisibleItems().filter((item) => item.type === type);
}
...
function replaceItemById(targetId, newItem) {
  ...
  items = items.map((item) =>
    item.id === targetId ? normalizedNewItem : item
  );
}
...
function getSubCategoryClass(subCategory) {
  if (subCategory === "觀看課程影片") return "video";
  if (subCategory === "練習") return "practice";
  if (subCategory === "寫筆記") return "note";
...
function getTaskEmptyMessage() {
  if (!getSelectedWeek()) return "目前還沒有可顯示的週次資料。";

  return isNextWeekView()
    ? "下週目前還沒有任務，先讓未來安靜排隊。"
    : "今天還沒立案也無妨，放一個小任務，就是好的開始。";
}
    ...
    if (normalizedItem.category === "程式學習") {
      meta.appendChild(
        createChip(
          ...
          `chip chip-subcategory ${getSubCategoryClass(
            normalizedItem.subCategory
          )}`
        )
      );
...
function renderProgress() {
  ...
  updateProgressFill(
    ...
    getProgressPercent(doneStandards.length, standards.length)
  );
}
...
function addTask() {
  addItem("task", taskInput, {
    ...
    subCategory: taskSubCategory ? taskSubCategory.value : DEFAULT_SUBCATEGORY,
...
async function initApp() {
  ...
  if (taskCategory) {
    taskCategory.addEventListener("change", renderSubCategoryControl);
  }
  ...
  if (currentWeekTab) {
    currentWeekTab.addEventListener("click", () => switchWeekView("current"));
  }

  if (nextWeekTab) {
    nextWeekTab.addEventListener("click", () => switchWeekView("next"));
  }
  ...
  standardInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") addStandard();
  });
